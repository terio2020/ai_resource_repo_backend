...
public class PackageController {
    ...
    public ResponseEntity<Result<PackageResponse>> create(
        ...
        return Result.ok("Package created", toResponse(ap));
    }

    @PutMapping("/{id}")
    @RequireAuth
    @Operation(summary = "Update package metadata", description = "Update description and tags of a package.")
    public ResponseEntity<Result<PackageResponse>> update(
    ...
    public ResponseEntity<Result<Void>> delete(
        ...
        return Result.okMessage("Package deleted");
    }
    ...
    public ResponseEntity<Result<List<PackageResponse>>> listPublic(
    ...
    public ResponseEntity<Result<List<PackageResponse>>> search(
    ...
    public ResponseEntity<Result<List<PackageResponse>>> listByAgent(
    ...
    public ResponseEntity<Result<List<PackageResponse>>> listByUser(
    ...
    public ResponseEntity<Result<PackageVersionResponse>> publishVersion(
            @Parameter(description = "Package ID") @PathVariable @Min(1) Long id,
    ...
    public ResponseEntity<Result<List<PackageVersionResponse>>> getVersions(
            @Parameter(description = "Package ID") @PathVariable @Min(1) Long id) {
    ...
    public ResponseEntity<Result<PackageVersionResponse>> getVersionDetail(
    ...
    public ResponseEntity<Result<List<PackageFileResponse>>> getVersionFiles(
            @Parameter(description = "Version ID") @PathVariable @Min(1) Long versionId) {
    ...
    public ResponseEntity<Result<Void>> setVisibility(
        ...
        return Result.okMessage(isPublic ? "Package set to public" : "Package set to private");
    }

    @PostMapping("/{id}/rollback")
    @RequireAuth
    @Operation(summary = "Rollback to a version", description = "Rollback the package to a previous version.")
    public ResponseEntity<Result<Void>> rollback(
            @Parameter(description = "Package ID") @PathVariable @Min(1) Long id,
            @Valid @RequestBody RollbackRequest request,
            HttpServletRequest httpRequest) {
        Long userId = (Long) httpRequest.getAttribute("userId");
        packageService.rollback(id, userId, request.getVersionId());
        return Result.okMessage("Rolled back successfully");
    }
